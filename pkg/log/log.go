...
func ClearLogs(a ...interface{}) {
	logDir := PATH
	days := DEFAULT_MAX_RETAIN_DAYS
	...
	for _, file := range files {
		...
		timeStr := fmt.Sprintf("%s %s", names[0], names[1])
		createTime, err := time.Parse("2006-01-02 15:04:05", strings.ReplaceAll(timeStr, ".", ":"))

		if err != nil {
			return
		}
		if createTime.Before(time.Now().Add(-time.Duration(days*24) * time.Hour)) {
